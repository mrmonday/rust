...
impl RawSocket {
    ...
    pub fn recvfrom(&mut self, buf: &mut [u8]) -> IoResult<(uint, Option<Box<NetworkAddress>>)> {
    ...
    pub fn sendto(&mut self, buf: &[u8], dst: Box<NetworkAddress>) -> IoResult<uint> {
...
pub enum NetworkAddress {
    ...
    IpAddress(IpAddr),
    /// A network interface
    NetworkAddress(Box<NetworkInterface>)
}
...
pub mod test {
    ...
    pub fn layer4_test(ip: IpAddr, headerLen: uint) {
        ...
        match RawSocket::new(get_proto(ip)) {
            Ok(mut sock) => match sock.sendto(message.as_bytes(), Box<IpAddress(ip)>) {
    ...
    iotest!(fn layer3_ipv4_test() {
        ...
        match RawSocket::new(NetworkProtocol(Ipv4NetworkProtocol)) {
            Ok(mut sock) => match sock.sendto(packet, Box<IpAddress(sendAddr)>) {
    ...
    iotest!(fn layer2_cooked_test() {
                ...
                Ok(mut sock) => {
                    ...
                    loop {
                        match sock.recvfrom(buf) {
                            Ok((len, Some(Box<NetworkAddress(ni)>))) => {
        ...
        match RawSocket::new(DataLinkProtocol(CookedEthernetProtocol(EtherTypes::Ipv4))) {
            Ok(mut sock) => {
                rx.recv();
                match sock.sendto(packet, box NetworkAddress(box interface2)) {
                    Ok(res) => assert_eq!(res as uint, packet.len()),
                    Err(_) => fail!()
                }
            },
