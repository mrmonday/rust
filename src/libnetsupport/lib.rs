...
pub fn sockaddr_to_network_addr(sa: *const libc::sockaddr, useLocal: bool) -> Option<Box<NetworkAddress>> {
    ...
    fn sll_to_ni(sll: libc::sockaddr_ll) -> Box<NetworkInterface> {
...
#[cfg(target_os = "linux")]
pub fn network_addr_to_sockaddr(na: Box<NetworkAddress>) -> (libc::sockaddr_storage, uint) {
    unsafe {
        match na {
            box IpAddress(ip) => addr_to_sockaddr(ip::SocketAddr { ip: ip, port : 0}),
            //_ => (mem::init(), 0)
...
pub fn sockaddr_to_network_addrs(sa: *const libc::sockaddr)
    ...
    match sockaddr_to_network_addr(sa, false) {
        ...
        Some(box IpAddress(ip@Ipv6Addr(..))) => (None, None, Some(ip)),
        Some(box NetworkAddress(ni)) => (ni.mac, None, None),
        None => (None, None, None)
    }
}
...
pub fn get_network_interfaces() -> Vec<Box<NetworkInterface>> {
    ...
    fn merge(old: &mut Box<NetworkInterface>, new: &Box<NetworkInterface>) {
